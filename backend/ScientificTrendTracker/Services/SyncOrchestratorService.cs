...
namespace ScientificTrendTracker.Services
{
    public class SyncOrchestratorService : ISyncOrchestratorService
    {
        ...
        private async Task SaveAuthorsAsync(OpenAlexPaper paper, string paperId)
        {
            ...
            // Chống trùng trong CÙNG 1 bài: OpenAlex đôi khi liệt kê 1 tác giả nhiều lần
            // → tránh lỗi EF "PaperAuthor ... same key {PaperId, AuthorId} already tracked".
            ...
            foreach (var authorship in paper.Authorships)
            {
                ...
                // Bỏ qua nếu tác giả này đã được thêm cho bài (trùng trong payload hoặc đã có dưới DB).
                if (!seenAuthorIds.Add(author.AuthorId)) continue;
                bool alreadyLinked = await _dbContext.PaperAuthors
                    .AnyAsync(pa => pa.PaperId == paperId && pa.AuthorId == author.AuthorId);
