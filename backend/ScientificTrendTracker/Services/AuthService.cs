...
namespace ScientificTrendTracker.Services
{
    ...
    public class AuthService : IAuthService
    {
        ...
        public async Task<string?> SendOtpAsync(SendOtpRequestDto request)
        {
            ...
            // Gửi mail là best-effort: OTP đã nằm trong cache nên KHÔNG để lỗi SMTP (vd: cấu hình
            // EmailSettings còn là giá trị dummy lúc dev) làm sập luồng đăng ký. Lấy OTP từ console log.
            try
            {
            ...
            {
                _logger.LogWarning(ex, "Gửi OTP qua SMTP thất bại — dùng MOCK OTP ở console log để tiếp tục.");
            }
        ...
        {
            // Logout là best-effort: nếu thiếu AccessToken hoặc token sai cấu trúc thì coi như
            // không có session hợp lệ để thu hồi → trả false thay vì ném 500 (IDX10000 token null).
            if (request == null || string.IsNullOrWhiteSpace(request.AccessToken)) return false;
        ...
        /// <summary>
        /// Lấy hồ sơ user hiện tại đọc thẳng từ DB theo UserId để FE hiển thị đúng dữ liệu gốc.
        /// </summary>
        ...
        public async Task<string?> SendForgotPasswordOtpAsync(SendOtpRequestDto request)
        {
            ...
            {
                _logger.LogWarning(ex, "Gửi OTP khôi phục qua SMTP thất bại — dùng MOCK OTP ở console log để tiếp tục.");
            }
